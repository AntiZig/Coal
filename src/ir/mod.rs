...
use ir_types::{IRBlock, IROp, IRValue, IReg, LocalCtx};
...
fn stmt_to_ir(stmt: Stmt, ctx: &mut LocalCtx) -> IReg {
    match stmt {
...
fn declare_to_ir(var: String, expr: Expr, ctx: &mut LocalCtx) -> IReg {
...
fn assign_to_ir(var: String, expr: Expr, ctx: &mut LocalCtx) -> IReg {
    let r1 = expr_to_ir(expr, ctx);
    let op = IROp::Store(r1, String::from("store into ".to_string() + &var));
    ctx.ops.push(op);
    let reg = ctx.ops.len() - 1;
...
fn expr_to_ir(e: Expr, ctx: &mut LocalCtx) -> IReg {
    match e {
...
fn mul_to_ir(e1: Box<Expr>, e2: Box<Expr>, ctx: &mut LocalCtx) -> IReg {
...
fn sub_to_ir(e1: Box<Expr>, e2: Box<Expr>, ctx: &mut LocalCtx) -> IReg {
    let r1 = expr_to_ir(*e1, ctx);
    let r2 = expr_to_ir(*e2, ctx);
...
fn add_to_ir(e1: Box<Expr>, e2: Box<Expr>, ctx: &mut LocalCtx) -> IReg {
    let r1 = expr_to_ir(*e1, ctx);
    let r2 = expr_to_ir(*e2, ctx);
...
fn fcall_to_ir(label: String, exprs: Vec<Expr>, ctx: &mut LocalCtx) -> IReg {
...
fn var_to_ir(var: String, ctx: &mut LocalCtx) -> IReg {
...
#[cfg(test)]
mod tests {
    use crate::code_gen::local_alloc;
    ...
    #[test]
    fn second_printing_test() {
        let func = get_ast_code_gen();
        let ret = fn_to_ir(func);
        println!("{:?}", ret);
        println!("{}", ret.to_string());
